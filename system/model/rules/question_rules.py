...
def rules_check(answer, queries, study_kb, user_kb):
    ...
    # Label France Education has no subject requirements NOTE: the question is printed here, to queries.addQuestion(
    # ), this 'if' should be separated into two, see below examples
    ...
    # Lise Diploma NOTE: the questions are printed here, to queries.addQuestion(), this 'if' should be separated into
    # multiple sections, see below examples
    if user_kb['diplomas'] == 'Lise Diploma' and 'subjects' not in user_kb.keys():
        ...
        if AP_extra == 'yes':
            ...
            print(
                "\nIf you have obtained a regular Lise Diplomasi you must have successfully completed at least your "
                "first year at an accredited technical university in a relevant program to meet our admission "
                "requirements. List of the accredited technical universities.")
            print(
                "ABDULLAH GÜL ÜNİVERSİTESİ\nACIBADEM MEHMET ALİ AYDINLAR ÜNİVERSİTESİ\nADANA ALPARSLAN TÜRKEŞ BİLİM "
            ...
            if approved_school == 'no':
                ...
                print(
                    "\nNote that:\nA relevant program is a program with Mathematics (and for some TU/e programs also "
                    "Physics/Chemistry) at the Dutch pre-university (VWO) level. Your Grade Point Average for the "
                    "first year must be at least 75%.\n")
    ...
    if 'subjects' not in user_kb.keys() and user_kb['diplomas'] != 'Label France Education':
        ...
        user_kb['subjects'] = []
        queries.add_question("----> What subjects did you take?\n choose from chemistry, physics, and mathematics?")
        return
    ...
    if 'subjectGrades' not in user_kb.keys() and 'subjects' in user_kb.keys() and user_kb['subjects'] != []:
        ...
        for study in study_kb:
            if any(isinstance(subjGrd, list) for subjGrd in
                   study[user_kb['diplomas']]):  # True if nested lists, means subjects have grades
                # print("NESTED")
    ...
    if 'subjectGrades' in user_kb.keys() and 'english level' not in user_kb.keys() and user_kb['subjects'] != []:
        temp_subj = [user_kb['subjects'][0]]
        grade = answer
        temp_subj.append(grade)
        user_kb['subjectGrades'].insert(0, temp_subj)
    ...
    if 'english level' not in user_kb.keys() and user_kb['diplomas'] not in ['IB', 'EB'] and user_kb['subjectGrades'] != []:
        ...
        queries.add_question("What is your English level?")
            ...
            if user_kb['english level'] is not None and user_kb['english level'] in study.keys() and 'english grades' not in user_kb.keys():
                ...
                for testGrd in study[user_kb['english level']]:
                    if testGrd[0] != 'overall':
                        ...
                        queries.add_question("Enter overall score for {}".format(user_kb['english level']))
